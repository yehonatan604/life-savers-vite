...
class DomService {
  ...
  links = () =>
    document.querySelectorAll(".link")! as NodeListOf<HTMLSpanElement>;
  images = () =>
    document.querySelectorAll("img") as NodeListOf<HTMLImageElement>;
  articles = () =>
    document.querySelectorAll("article") as NodeListOf<HTMLElement>;
  ...
  private constructor() {
    ...
    hamburger.addEventListener("click", () => {
      ...
      !header.classList.contains("after")
        ? header.classList.add("after")
        : header.classList.remove("after");
      main.classList.contains("h-[90vh]")
        ? main.classList.replace("h-[90vh]", "h-[72.5vh]")
        : main.classList.replace("h-[72.5vh]", "h-[90vh]");
    });
    links.forEach((link) => {
